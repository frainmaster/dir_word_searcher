...
def readfile(path, i):
    with open(path + '/' + i, 'r') as file:
        ...
        for a, lines in enumerate(file):
            lowered = lines if 'c' in option else lines.lower()
            if len(keyword) > 1 and 'm' in option:
                if all(ii in lowered for ii in k):
                    if 'j' in option:
                        print('file: {}'.format((path + '/' + i).replace(oriPath, '')))
                        break
                    print('file: {}, line: {}'.format((path + '/' + i).replace(oriPath, ''), a + 1))
            else:
                if any(j in lowered for j in k):
                    k = [j for j in k if j not in lowered]
                if len(k) == 0:
                    print('file: {}, line: {}'.format((path + '/' + i).replace(oriPath, ''), a + 1))
                    break
...
if __name__ == '__main__':
    while True:
        ...
        oriPath = ''
        '''
        works on CLI level. accepts output as:
            <file_name> <space-separated searched words> --path="<file path>" --option=<options>
        '''
        if len(sys.argv[1:]) > 0:
            keyword = []
            oriPath = os.getcwd()
